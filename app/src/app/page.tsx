...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Throughput" value={kpiVal('Throughput', '242K bpd')} status="neutral" />
        <KPICard title="Solomon EII" value={kpiVal('Solomon EII', '94.2')} status="neutral" />
        <KPICard title="Unplanned Downtime" value={kpiVal('Unplanned Downtime', '0.8%')} status="neutral" />
        <KPICard title="Product Yield" value={kpiVal('Product Yield', '87%')} status="neutral" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 grid-cols-1">
        <Chart
          ...
          type="line"
          xKey="period"
          yKeys={[{ key: 'value', name: 'K bpd' }]}
          title="Throughput vs Plan (Daily)"
        />
        <Chart
          ...
          type="bar"
          xKey="category"
          yKeys={[{ key: 'count', name: 'Volume (K bpd)' }]}
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="Margin Optimization" value={kpiVal('Margin Optimization', '+RM 2.4M/day')} />
        <KPICard title="Energy Index" value={kpiVal('Energy Index', '92')} />
        <KPICard title="Hydrogen Balance" value={kpiVal('Hydrogen Balance', '98%')} />
      </div>
      <Chart
        data={data?.detail || [{ x: 'Mon', y: 24 }, { x: 'Tue', y: 28 }, { x: 'Wed', y: 22 }, { x: 'Thu', y: 31 }, { x: 'Fri', y: 26 }, { x: 'Sat', y: 19 }, { x: 'Sun', y: 23 }]}
        type="area"
        xKey="x"
        yKeys={[{ key: 'y', name: 'RM/bbl' }]}
        title="Refinery Margin Trend (Weekly)"
        height={400}
      />
    </div>
  );
  ...
  const domainTab2 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        />
        <ActionMemo
          persona={{ name: 'Ir. Mohd Faizal', role: 'Refinery Operations Manager' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Optimize CDU feed blend for current crude slate', 'Schedule turnaround for RFCC catalyst change', 'Investigate heat exchanger fouling in Unit-4'],
              },
              {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which units are below target throughput?',
          'Show refinery margin sensitivity to crude price',
          'What is the optimal product slate for current crack spreads?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Optimization', icon: '📈', content: domainTab1 },
    { id: 'domain-2', label: 'Reliability', icon: '⚡', content: domainTab2 },
